...
import { contentEditPath } from '../../../shared/calendar'
import {
...
function editPath(contentType: CalendarContentType, id: number): string {
  return contentEditPath(contentType, id)
}
...
async function fetchHealth(db: AppDb, locale: string): Promise<DashboardHealthCounts> {
  const blobs = createBlobQueries(db)
...
export function createDashboardQueries(db: AppDb) {
  return {
    buildPipeline: (locale: string) => buildPipeline(db, locale),
    listMergedBacklog: (locale: string, limit: number) => listMergedBacklog(db, locale, limit),
    listRecentlyUpdated: (locale: string, limit: number) => listRecentlyUpdated(db, locale, limit),
    fetchHealth: (locale: string) => fetchHealth(db, locale),
    fetchLastPublished: (locale: string) => fetchLastPublished(db, locale),
  }
}
